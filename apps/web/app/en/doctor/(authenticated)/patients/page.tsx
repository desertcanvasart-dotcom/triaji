...
export default function GPPatientPanelPage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6" dir="ltr">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#1A2F4A]">My Patients</h1>
        <p className="text-gray-500 mt-1">GP patient panel overview</p>
      </div>
      ...
      {filteredPatients.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          <p className="text-gray-400 text-lg">
            {filter === 'attention' ? 'No patients need urgent attention' : 'No patients'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredPatients.map(patient => {
            ...
            return (
              <Link
                key={patient.id}
                href={`/en/doctor/patients/${patient.id}`}
                className={`block bg-white rounded-xl border p-5 hover:shadow-md transition-shadow ${
                  patient.needsAttention ? 'border-amber-300 border-l-4 border-l-amber-500' : 'border-gray-200'
                }`}
              >
                {/* Patient header */}
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#1A2F4A] text-sm">{displayName}</p>
                      <p className="text-xs text-gray-400">
                        {patient.profile?.age ? `${patient.profile.age} y/o` : ''}
                        {patient.profile?.age && patient.biologicalSex ? ' - ' : ''}
                        {patient.biologicalSex === 'male' ? 'Male' : patient.biologicalSex === 'female' ? 'Female' : ''}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {patient.protocols.map(p => (
                      <span key={p.protocolId} className="px-2 py-0.5 bg-purple-50 text-purple-700 text-xs rounded-full">
                        {p.nameEn ?? p.nameAr ?? p.conditionCode ?? ''}
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  {patient.overdueFollowUps > 0 && (
                    <span className="flex items-center gap-1 text-red-600">
                      ...
                      {patient.overdueFollowUps} overdue
                    </span>
                  )}
                  {patient.profile?.bmi && (
                    <span className="text-gray-400">BMI: {patient.profile.bmi}</span>
                  )}
                </div>
              </Link>
            );
