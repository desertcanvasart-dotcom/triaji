    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <header className="bg-teal-700 text-white py-8 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {config?.clinic_specialty_ar && (
            <p className="text-teal-100 mt-1">{config.clinic_specialty_ar}</p>
          )}
        </div>
      </header>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">معلومات العيادة</h2>

          {config?.opening_time && config?.closing_time && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <span>🕐</span>
              <span>
                ساعات العمل: {config.opening_time} — {config.closing_time}
              </span>
            </div>
          )}
          ...
          {config?.clinic_phone && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <span>📞</span>
              <a href={`tel:${config.clinic_phone}`} dir="ltr" className="text-teal-600 hover:underline">
                {config.clinic_phone}
              </a>
            </div>
          )}
        ...
        {bookingMode === 'walk_in_only' && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-center">
            ...
            <p className="text-sm font-medium text-amber-800">
              هذه العيادة تعمل بنظام الانتظار
            </p>
            <p className="text-xs text-amber-600 mt-1">
              حضّر وقتك ووصل في أوقات العمل
            </p>
          </div>
        )}
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900">الأطباء</h2>
          {(doctors ?? []).map((doctor) => (
        ...
        {(bookingMode === 'slots_only' || bookingMode === 'both') && (
          <ClinicBookingClient
            tenantId={tenant.id}
            clinicName={tenant.name_ar}
            doctors={(doctors ?? []).map((d) => ({
              id: d.id as string,
              name_ar: d.name_ar as string,
              title_ar: (d.title_ar as string) ?? 'د.',
            }))}
        ...
        {bookingMode === 'both' && (
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
            <p className="text-sm text-gray-500">
              أو تقدر تيجي من غير موعد وتنضم لقائمة الانتظار 🚶
            </p>
          </div>
        )}
      </div>
