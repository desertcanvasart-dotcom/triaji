/**
 * POST /api/webhooks/lab-chain/alfa
 *
 * Receives Alfa Lab result webhooks.
...
const CHAIN_CODE = 'alfa' as const;
...
export async function POST(request: NextRequest) {
  ...
  const signature = request.headers.get('x-alfa-signature') ?? '';
  const timestamp = request.headers.get('x-alfa-timestamp') ?? '';
  ...
  if (!isValid) {
    console.error(`[webhook/alfa] Invalid signature`);
  ...
  await supabase
    ...
    .then(({ error }) => {
      if (error) console.error(`[webhook/alfa] Failed to log webhook:`, error);
    });

  // ─── Find routing record ───────────────────────────────────────────────────
  if (!chainOrderId) {
    console.log(`[webhook/alfa] No order_id in payload, event: ${eventType}`);
  ...
  if (!routing) {
    console.warn(`[webhook/alfa] No routing found for chain_order_id: ${chainOrderId}`);
  ...
  if (eventType === 'results_ready' || eventType === 'results_partial') {
    try {
      ...
      console.error(`[webhook/alfa] Failed to process results for ${chainOrderId}:`, err);
      await supabase
        .from('lab_chain_webhooks')
