...
export default function LabDashboard({ tenantId }: { tenantId: string }) {
  const [stats, setStats] = useState<LabStatsResponse | null>(null);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/lab/stats', { headers });
      if (res.ok) {
  ...
  const statCards = [
    ...
    {
      label: 'متوسط وقت النتيجة',
      value: data.avg_turnaround_hours > 0 ? `${data.avg_turnaround_hours} س` : '—',
      icon: '⏱',
