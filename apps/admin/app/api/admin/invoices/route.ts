...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    ...
    .select('*, doctors:doctors(name_ar, name_en)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  ...
  const {
    patient_name_ar, doctor_id, line_items,
    discount_egp = 0, insurance_covered = 0,
    payment_method, queue_entry_id, booking_id,
    patient_id, notes_ar, status: invoiceStatus,
  } = body;

  if (!patient_name_ar || !doctor_id || !line_items) {
    return NextResponse.json({ error: 'patient_name_ar, doctor_id, and line_items are required' }, { status: 400 });
  }
  ...
  if (rpcError) {
    return NextResponse.json({ error: `Failed to generate invoice number: ${rpcError.message}` }, { status: 500 });
  }
